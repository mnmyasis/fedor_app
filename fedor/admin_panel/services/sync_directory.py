...
def __request_east_api(page):
    ...
    variables = {"status": 1, 'page': page, 'count': 100}
...
def start_sync():
    ...
    threads = []
    for i in range(2):
        x = threading.Thread(target=__request_east_api, args=(i,))
        threads.append(x)
        x.start()
...
def request_eas_api():
    ...
    page = 0
    while api_status:
        variables = {"status": 1, 'page': page, 'count': 500}
        data = client.execute(query=query, variables=variables)
        eas_list = data.get('data').get('eas_product')
        if (eas_list):
            for eas_data in eas_list:
...
def request_sku_api():
    ...
    for sku_data in sku_list:
        ...
        s_sku, s_status = SyncSKU.objects.update_or_create(**sku_wr, defaults=sku_wr)
        print(s_sku)
    return True
